  ...
  return (
    ...
    >
      <Link href={`/produkty/${product.slug}`} className="block">
        ...
        <div className="p-4">
          {product.categories && (
            <p className="text-xs font-medium uppercase tracking-wider mb-1" style={{ color: '#c5167a' }}>
              {product.categories.name}
            </p>
          )}
          <h3 className="font-semibold text-gray-900 mb-2 line-clamp-2 leading-tight">{product.name}</h3>
          <div className="flex items-center justify-between">
            <p className="text-lg font-bold" style={{ color: '#e91e8c' }}>
      ...
      <div className="px-4 pb-4">
        <button
          ...
          style={{
            background: product.stock > 0 ? 'linear-gradient(135deg, #e91e8c, #8b0048)' : '#e5e7eb',
            color: product.stock > 0 ? '#080808' : '#6b7280',
          }}
        >
